...
**Hint cube**: an optional small solid cube (20–80 mm) whose faces show the puzzle's color scheme as plain **nominal 1×2 rectangles** — colors only, no piece shapes, so it helps sort pieces by face without spoiling the cutting. A core body (the slicer gives it regular infill) plus four color tile sets flush-embedded into the surface. By default the mosaics run edge to edge (colliding surface layers along the cube edges are resolved with a fixed face priority, so a tile-depth-thin strip of the neighbor face's colors shows along each edge — much like the real cube's edge teeth); an optional **frame** checkbox switches to a thin core-colored border around each face instead. Two export options:
