...
- Every border between two pieces is drawn from the space of **profiles** — per-position offsets from {−1, 0, +1}. Profiles must carry at least two tooth steps on their interior, cube-edge contact windows must alternate at least twice, and the generator validates the result: every long seam keeps at least two snap-carrying tooth flanks, and every piece is snap-held on at least two of its contacts (a piece held at a single point would fall out of a partly assembled cube) — otherwise the cutting is rerolled. Every piece outline is unique (verified against all rotations and reflections — duplicates are regenerated). All teeth are straight, so every joint — in-face or across a cube edge — looks identical.
...
**Text on faces**: any face can carry a multi-line inscription — engraved (pockets) or embossed (raised letters) — with 3×3 alignment options and a choice of four typefaces (bold sans, Lobster script, Caveat handwriting, Playfair serif — all with Cyrillic support). Text is rasterized into a pixel mask whose resolution adapts to the print: the subpixel is kept near the nozzle width (~0.42 mm), so even a difficulty-1 face gets a fine enough grid for calligraphic letters. The relief is 0.2 cell deep, gets split across the puzzle pieces, keeps one cell clear of the cube edges, and travels inside the share link (font included). Pieces carry the relief on their top side, so in flat print mode both styles print letters-up, and the assembled cube matches the net preview exactly.
...
**Hint cube**: an optional small solid cube (20–80 mm) whose faces show the puzzle's color scheme as plain **nominal 1×2 rectangles** — colors only, no piece shapes, so it helps sort pieces by face without spoiling the cutting. A core body (the slicer gives it regular infill) plus four color tile sets flush-embedded into the surface. By default the mosaics run edge to edge with the color layers **mitered at 45°** along the cube edges like a picture frame — every face fully owns its visible surface, border tiles carry a 45° bevel on their underside, and the bevels of adjacent faces meet flush in the miter plane through the edge. An optional **frame** checkbox switches to a thin core-colored border around each face instead. Enabling the hint cube also adds a **HINT CUBE** preview tab next to NET/PLATES: two interactive, synchronously rotating 3D cubes — the **assembled puzzle** (real piece shapes, interlocked edge teeth included) and the hint cube mosaic. Drag to rotate; a slow turntable spin resumes when idle. Side by side on desktop, stacked on mobile. Two export options:
