...
- Borders between neighboring elements wander randomly by **−1/0/+1 cells**, so every piece gets a unique pixelated outline (verified against all rotations and reflections — duplicates are regenerated). All teeth are straight, so every joint — in-face or across a cube edge — looks identical. Every border is guaranteed to have at least one interlocking step (perfectly straight contacts are re-wiggled).
- Every pair of neighboring pieces gets a **snap fixator**: a small semicircular ridge on one piece clicks into a matching undercut groove on the other. The ridge side is chosen randomly but deterministically — the same seed always reproduces the same model. In flat mode pieces mate at zero clearance; the 45° tilted export adds a 0.1 mm/side clearance automatically (tilted prints run tighter), and the snap keeps the pieces together.
